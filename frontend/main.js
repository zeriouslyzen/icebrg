...
const API_URL = env.VITE_API_URL || `/api/query`;
const WS_URL = env.VITE_WS_URL || `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/ws`;
...
(function () {
    ...
    function handleLockedClick(e) {
        ...
        if (lockedModule) {
            // Paywall disabled: allow navigation
            const targetUrl = lockedModule.getAttribute('data-original-href') || lockedModule.getAttribute('href');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // Clean ?error=locked from URL (paywall disabled)
        const urlParams = new URLSearchParams(window.location.search);
